...
class WhatsAppFlowTrainerOnboarding:
    ...
    def __init__(self, supabase, whatsapp_service):
        ...
        # Get flow_id and flow_name from environment variables with defaults
        self.flow_id = os.getenv('TRAINER_ONBOARDING_FLOW_ID', '775047838492907')
    ...
    # called after flow subsmisstions from routes/webhooks.py (interactive_type == 'nfm_reply') -> flow_handlers\flow_response_handler.py (flow_token and 'trainer_onboarding' in flow_token or flow_name == 'trainer_onboarding_flow')
    def process_flow_completion(self, flow_data: Dict, phone_number: str) -> Dict:
        ...
        try:
            ...
            # Extract available_days and preferred_time_slots from working_hours
            available_days = self._extract_available_days(working_hours)
            preferred_time_slots = self._extract_preferred_time_slots(working_hours)
            
            # Parse experience years to numeric value
            years_experience = self._parse_experience_years(experience_years) if experience_years else 0
            
            # Prepare registration data dict with all processed fields
            registration_data = {
                ...
                'experience_years': experience_years,  # Original string
                'years_experience': years_experience,  # Parsed numeric value
                'pricing_per_session': pricing_per_session,
                'working_hours': working_hours,  # JSONB structure
                'available_days': available_days,  # Extracted list
                'preferred_time_slots': preferred_time_slots,  # Extracted string
    ...
    # to not register with existing trainer's phone number
    ...
    # to not register with existing trainer's email
    ...
    # Build working_hours JSONB structure from flow availability flow days data
